...
(function($, window) {
    ...
    PickList.DEFAULTS = {
        ...
        //   buttons: [{ action: 'add', label: ' ＞ ' }, { action: 'addAll', label: '＞＞' }, { action: 'remove', label: ' ＜ ', className: 'btn btn-sm btn-block btn-info' }, { action: 'removeAll', label: '＜＜' }],
        buttons: [{ action: 'add', label: ' ＞ ', title: '添加' }, { action: 'addAll', label: '＞＞', title: '全選' }, { action: 'remove', label: ' ＜ ', title: '移除' }, { action: 'removeAll', label: '＜＜', title: '重選' }],
    ...
    PickList.prototype.initButtons = function() {
        ...
        $.each(this.options.buttons, function(i, v) {
            ...
            button.attr('title', typeof v.title == 'undefined' ? v.action : v.title);
